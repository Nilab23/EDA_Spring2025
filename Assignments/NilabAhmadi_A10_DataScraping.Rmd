...
months <- c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug",
            "Sep", "Oct", "Nov", "Dec")
years <- rep(2020, times = 12)  # Change the year here if needed
...
ggplot(water_data, aes(x = Month, y = MaxDayUseMGD, group = 1)) +
  geom_line() +  
  geom_point() +  
  labs(title = "Maximum Daily Water Withdrawals in 2024",
       x = "Month",
...
# 6. Creates a URL pointing to the LWSP for that PWSID for the given year
...
#7 
durham_data_2020 <- scrape_data_function('03-32-010', 2020)


ggplot(durham_data_2020, aes(x = Date, y = MaxDayUsage)) +
  geom_line() +
  geom_point() +
  labs(title = "Maximum Daily Water Withdrawals for Durham in 2020",
       x = "Date",
       y = "Max Day Use (MGD)") +
  theme_minimal() +
...
#8 
# Fetch data for Asheville (PWSID '01-11-010') in 2020
asheville_data_2020 <- scrape_data_function('01-11-010', 2020)

# Combine the Durham data and Asheville data into one dataframe
combined_data <- bind_rows(asheville_data_2020, durham_data_2020)
...
ggplot(combined_data, aes(x = Date, y = MaxDayUsage, color = WaterSystemName)) +
  ...
  geom_point() + 
  labs(
    title = "Comparison of Monthly Maximum Daily Water Usage in 2020",
    subtitle = "Asheville vs. Durham",
    x = "Date",
    y = "Max Day Usage (MGD)"
  ) +
  theme_minimal() +  
  scale_color_manual(values = c("Asheville" = "green", "Durham" = "darkblue")) +  
  scale_x_date(
...
ggplot(asheville_data, aes(x = Date, y = MaxDayUsage)) +
  ...
  labs(title = "Asheville's Monthly Maximum Daily Water Usage (2018-2023)",
       x = "Date",
       y = "Max Day Usage (MGD)") +
  theme_minimal() +
...
> Answer:>Based on the plot, Asheville's water usage shows a slight upward 
trend from 2018 to 2023, indicating a gradual increase in demand over time.
>
